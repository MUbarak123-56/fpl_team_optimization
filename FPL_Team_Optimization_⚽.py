...
st.set_page_config(page_title = "FPL Team Optimization", page_icon="⚽")
...
def plot_formation(line_up, line_up2):
    ...
    gk_points = line_up[line_up["position"]=="GK"]["points_per_game"].to_list()
    def_points = line_up[line_up["position"]=="DEF"]["points_per_game"].to_list()
    mid_points = line_up[line_up["position"]=="MID"]["points_per_game"].to_list()
    fwd_points = line_up[line_up["position"]=="FWD"]["points_per_game"].to_list()
    ...
    ax.text(50, 95, "Total Points per game for Starting line up: " + str(round(line_up["points_per_game"].sum(),2)) + "; " + "Total Squad Budget: " + str(round(line_up2['value'].sum(), 2)), color="white", fontsize=7, fontweight="bold")
...
def plot_bench(bench):
    ...
    points=bench["points_per_game"].tolist()
...
if 'API_TOKEN' in st.secrets:
            ...
            #or len(api_key) != 45:
                        ...
                        with st.spinner('Please wait..Squad is being selected'):
                            ...
                            team_df = team_df[(team_df['variable'].str.startswith('x', na=False)) & (team_df['selected'] == 1)]
                            ...
                            plot_formation(start_lineup_df, team_df)
